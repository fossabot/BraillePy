...
from character import Char, InvalidUnicodeCodePoint
...
class InvalidBrailleCodepoint(InvalidUnicodeCodePoint):
	pass
...
class Braille(Char):
	def __init__(self, value: Union[list, int, str] = ([
		...
		self.MIN: int = BRAILLECHAROFFSET
		self.MAX: int = BRAILLECHARMAX
		self.RANGE: list = [u for u in range(self.MIN, self.MAX + 1)]
...
def _BRAILLE_TO_STRING(BRAILLE: Union[Braille, list]) -> str:
	...
	if isinstance(BRAILLE, Braille) or isinstance(BRAILLE, Char) and hasattr(BRAILLE, 'dots_map'):
